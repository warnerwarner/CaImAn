...
def volspike(pars):
    ...
    args = pars[5]
    window_length = int(
        fr * args["template_size"]
    )  # half window length for spike templates
    output = {}
    output["rawROI"] = {}
    ...
    bwexp = dilation(
        bw, _shift_footprint_old(np.ones([args["context_size"], args["context_size"]]))
    )
    ...
    if weights_init is None:
        ...
        print("Reuse weights")
        # crop full-FOV weights to context region if needed
        w = np.reshape(weights_init, images.shape[1:])
        w = w[Xinds[0] : Xinds[-1] + 1, Yinds[0] : Yinds[-1] + 1]
        t0 = np.matmul(data_hp, w.ravel())  # reuse weights
    t0 = t0 - np.mean(t0)
    ...
    alpha = (
        args["nPC_bg"] * args["ridge_bg"]
    )  # square of F-norm of Ub is equal to number of principal components
    ...
    if len(spikes) > 0:
        output["rawROI"]["t"] = (
            output["rawROI"]["t"]
            * np.mean(t0[output["rawROI"]["spikes"]])
            / np.mean(output["rawROI"]["t"][output["rawROI"]["spikes"]])
        )  # correct shrinkage
        ...
        (
            ...
            np.reshape(
                movie.gaussian_blur_2D(
                    np.reshape(pred, (data_hp.shape[0], ref.shape[0], ref.shape[1])),
                    kernel_size_x=7,
                    kernel_size_y=7,
                    kernel_std_x=1.5,
                    kernel_std_y=1.5,
    ...
    recon = np.hstack(
        (
            ...
            np.reshape(
                movie.gaussian_blur_2D(
                    np.reshape(recon, (data_hp.shape[0], ref.shape[0], ref.shape[1])),
    ...
    # skip iterative refinement if no spikes detected — singular matrix crash otherwise
    if len(spikes) == 0:
        t = t0.copy()
        weights = np.zeros((recon.shape[1],))
    else:
        for iteration in range(args["n_iter"]):
            ...
            # correct shrinkage — skip if degenerate (zero-weight fallback from singular NMF)
            t_mean_spike = np.mean(t[spikes])
            if np.abs(t_mean_spike) > 1e-15:
                weights = weights * np.mean(t0[spikes]) / t_mean_spike
                t = np.double(t * np.mean(t0[spikes]) / t_mean_spike)
    ...
    # locality test — skip if no spikes (t_rec all zeros, div by zero)
    if np.sum(np.abs(t_rec)) == 0:
        locality = False
    else:
    ...
    output["F0"] = np.abs(
        np.nanmean(data_lp[:, bw.flatten()] + output["mean_im"][bw][np.newaxis, :], 1)
    )
...
def denoise_spikes(
    ...
    # no peaks detected — return empty results
    if len(pks) == 0:
        return (
            data,
            np.array([], dtype=int),
    ...
    if threshold_method == "adaptive_threshold":
        thresh2, falsePosRate, detectionRate, low_spikes = adaptive_thresh(
            pks2, clip=0, pnorm=pnorm, min_spikes=min_spikes
        )  # clip=0 means no clipping
    ...
    if do_plot:
        ...
        plt.plot(
            locs,
            np.max(datafilt) * 1.1 * np.ones(locs.shape),
            color="r",
            marker="o",
            fillstyle="none",
            linestyle="none",
        ...
        plt.plot(
            spikes,
            np.max(datafilt) * 1 * np.ones(spikes.shape),
            color="g",
            marker="o",
            fillstyle="none",
            linestyle="none",
...
def adaptive_thresh(pks, clip, pnorm=0.5, min_spikes=10):
    ...
    lastpt = np.where(
        np.logical_and(csf[0:-1] > csmodel[0:-1] + np.spacing(1), csf[1:] < csmodel[1:])
    )[0]
    if not lastpt.size:
        lastpt = center
    else:
    ...
    if np.sum(pks > thresh) < min_spikes:
        low_spikes = True
        logging.warning(
            f"Few spikes were detected. Adjusting threshold to take {min_spikes} largest spikes"
        )
...
def whitened_matched_filter(data, locs, window):
    ...
    censor[locs] = 1
    censor = np.int16(
        np.convolve(censor.flatten(), np.ones([1, len(window)]).flatten(), "same")
    )
    censor = censor < 0.5
    noise = data[censor]

    nperseg = min(1000, len(noise))
    _, pxx = signal.welch(
        noise,
        fs=2 * np.pi,
    ...
    dd = (
        cv2.dft(cc, flags=cv2.DFT_SCALE + cv2.DFT_COMPLEX_OUTPUT)[:, 0, :]
        * scaling_vector[:, np.newaxis]
    )[:, np.newaxis, :]
...
def signal_filter(sg, freq, fr, order=3, mode="high"):
    ...
    sg = np.single(
        signal.filtfilt(b, a, sg, padtype="odd", padlen=3 * (max(len(b), len(a)) - 1))
    )
    return sg
